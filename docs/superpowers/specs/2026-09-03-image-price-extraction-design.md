...
## Task 3 실측 결과 (2026-09-03, 쿠키 갱신 후)

최초 시도 때는 `backend/.env`의 `NAVER_COOKIE`가 만료돼 있어 `article.cafe.naver.com`
호출이 401(`errorCode 0004, 로그인하지 않았습니다`)이었다. 쿠키 갱신 후 재시도:

- 스펙 근거 글(`cafe.naver.com/winerack24/367941`, GS25 15,920원 사례)로 article API
  호출 → 200, `img count 2`. 와쌉 글에 실제로 이미지가 들어있음을 확인.
- 이미지 CDN(`cafeptthumb-phinf.pstatic.net`) 다운로드 → **쿠키 없이도 200** —
  블로그 CDN(`mblogthumb-phinf`)과 마찬가지로 인증 불필요. `download_image()`의
  `cookie` 파라미터는 안 써도 되지만(현재 코드 그대로 둬도 무해함), article API
  자체(`fetch_wassap_full_body`)는 쿠키가 있어야 한다.
- 이 2장 중 하나가 실제 `결제하기` 화면("최종 결제 금액 15,920원")이었고, 이걸로
  Gemini/OCR을 재검증한 결과가 아래 "채택 결과 재검증"이다.

## 채택 결과 (2026-09-03)

| 추출기 | 라벨수 | 정답 | 오탐 | 미검출 | 실패 | 초/장 |
|---|---|---|---|---|---|---|
| gemini | 49 | 46 | 0 | 3 | 0 | 5.89 |
| ocr | 49 | 46 | 0 | 3 | 0 | 0.16 |

채택: `ocr` — 정답/오탐/미검출 지표가 완전히 동일한데(둘 다 라벨 있는 3장을 다 놓쳤다 —
아래 한계 참고), Gemini는 49장 순차 호출 중 무료 티어 `429 Too Many Requests`가 2번
발생했고 이미지당 5.89초로 OCR(0.16초)보다 37배 느리다. 스펙이 정한 기준(인식률·
rate limit) 중 인식률이 갈리지 않는 상황에서는 rate limit 없고 무료인 OCR을 우선한다.

**한계:** 라벨링한 3장(19,900 / 16,800 / 54,900원)은 전부 매대 가격표·상품 목록
캡처였다(할인율, 화살표 표기, 100ml당 단가 등) — `parse_price_from_ocr_text`가 요구하는
"결제/결제금액" 라벨이 없어 OCR·Gemini 둘 다 스킵했다(오탐 아님, 설계된 동작).
이 기능이 원래 잡으려던 **결제 화면 캡처**(스펙 배경의 GS25 15,920원 사례)는
Task 3에서 `NAVER_COOKIE` 만료로 와쌉 글을 못 가져와 벤치 세트에 못 넣었다.

## 채택 결과 재검증 (2026-09-03, 쿠키 갱신 후) — 결정 뒤집힘

쿠키 갱신 후 실제 근거 글(`cafe.naver.com/winerack24/367941`)의 진짜 결제화면
캡처 2장(이벤트 배너 1장 + `결제하기` 화면 1장, "최종 결제 금액 15,920원")을
가져와 두 추출기에 직접 돌렸다:

- **Gemini**: `15920` — 정확히 추출.
- **OCR(Tesseract kor+eng)**: `None` — 실패. 원인은 인식 자체: 이 앱 UI 폰트에서
  "최종 결제 금액" 라벨이 `^`, `wo`, `41` 같은 알아볼 수 없는 문자로 깨져서
  `_LABEL_RE`가 매칭할 라벨을 못 찾았다(금액 숫자 "15", ",920원" 자체는 다른 줄에
  읽혔지만 라벨 없이는 안 쓴다 — 설계대로 "지어내지 않음"이 작동한 것뿐, 그런데
  이 경우는 진짜 있는 값을 놓친 것).

즉 위 표의 "49장 벤치마크 무승부"는 매대 가격표만 있던 합성 샘플의 결과였고,
**정작 이 기능이 잡아야 할 실제 결제화면에서는 OCR이 통째로 실패**한다.
**채택을 `gemini`로 변경**한다. Rate limit(무료 티어 429, 이번 세션에서만 3번
관찰)은 여전한 리스크지만, 이미지 경로는 본문 텍스트에서 가격 0건일 때만 타므로
(설계 문서 "결정 사항" 참고) 실제 호출 빈도는 낮다 — rate limit보다 "아예 못
읽음"이 더 치명적이라고 판단.

`backend/.env`의 `IMAGE_PRICE_EXTRACTOR`를 `gemini`로 변경함.
